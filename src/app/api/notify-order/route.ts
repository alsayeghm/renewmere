import { NextResponse } from "next/server";
import { renderPlanStartedEmail, type EmailOrderItem } from "@/lib/email";

const NOTIFY_TO = "hello@renewmere.com";

export async function POST(request: Request) {
  const secret = request.headers.get("x-webhook-secret");
  if (secret !== process.env.ORDER_WEBHOOK_SECRET) {
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  }

  const resendKey = process.env.RESEND_API_KEY;
  if (!resendKey) {
    // Not configured yet — don't fail the DB trigger over it.
    return NextResponse.json({ skipped: "no RESEND_API_KEY" });
  }

  const body = await request.json();
  const {
    order_id: orderId,
    items,
    total_one_time_pence: totalOneTime,
    total_annual_pence: totalAnnual,
    total_recurring_pence: totalRecurring,
    requester_email: requesterEmail,
  }: {
    order_id: string;
    items: EmailOrderItem[];
    total_one_time_pence: number;
    total_annual_pence: number;
    total_recurring_pence: number;
    requester_email: string | null;
    check_id: string | null;
    created_at: string;
  } = body;

  const html = renderPlanStartedEmail({
    orderId,
    requesterEmail: requesterEmail ?? "unknown",
    items,
    totalOneTimePence: totalOneTime,
    totalAnnualPence: totalAnnual,
    totalRecurringPence: totalRecurring,
  });

  const res = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${resendKey}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      from: "Renewmere <notifications@renewmere.com>",
      to: [NOTIFY_TO],
      subject: `Checkout started (${items.length} item${items.length === 1 ? "" : "s"})`,
      html,
    }),
  });

  if (!res.ok) {
    const detail = await res.text();
    return NextResponse.json({ error: "resend_failed", detail }, { status: 502 });
  }

  return NextResponse.json({ sent: true });
}
